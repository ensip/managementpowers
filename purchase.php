<?php 
include('inc/network/PayTpvDubaiApi.php');
$prices = [
	'starter' => 20,
	'smb' => 40,
	'enterprise' => 60,
...
<section class="section-flexible section-html section-products">
	...
	<section style="width:450px;margin:0 auto;">
	<h2>Purchase <?=$_REQUEST['t']?></h2>
	<p style="text-align:center"><b style="color:black;">20€ or 86AED</b> / user / month<br>
	<em>(2 users minimum)</em></br>
	<small>*Amount non-refundable</small>
	</p>
	<form style="" action="purchase.php" method="POST">
	<label class="required" for="message">Phone number*</label>
		<input type="hidden" name="t" value="<?=$_REQUEST['t']?>">
		<input type="text" name="name" value="<?=$_POST['phone']?>"  style="background-color:#ddd;color:black;" disabled>
		<label class="required" for="message">Name*</label>
		<input type="text" name="name" value="<?=$_POST['name']?>"  style="background-color:#ddd;color:black;" disabled>
		<label class="required" for="message">Reference product*</label>
		<input type="" name="reference" value="<?=$_POST['reference']?>" style="background-color:#ddd;color:black;"  disabled>
	</form>
	</section>
	<section style="width:950px;margin:0 auto;" id="iframe">
<?php 
//syslog(LOG_INFO, __FILE__.": GET: ".print_r($_GET, true));
//syslog(LOG_INFO, __FILE__.": POST: ".print_r($_POST, true));
if ($purchase) {
        $opts['cbn_real'] = true;
        $servicio = 'ensip'; //para que funcione
        $usuario = 2;//para que funcione
        $divisa = 'EUR'; //usd
        $form_model = new StdClass;
        $form_model->iframe = true;
        $form_model->servicio = $servicio;
        $form_model->usuario = $usuario;
        $form_model->cantidad = $price;
        $form_model->divisa = $divisa;

        $paytpv_api = new PayTpvDubaiApi($usuario, $divisa, $servicio, $opts);
        $iframe = $paytpv_api->getIframePago($form_model, $servicio, $usuario);
        if ($iframe !== false) { ?>
                <iframe id="iframe_tarjeta_guardada" name="iframe_tarjeta_guardada" src="<?=$iframe?>" style="width: 900px; height: 900px; border: 0;" sandbox="allow-top-navigation allow-scripts allow-popups allow-forms allow-same-origin"></iframe>
<?php   }
